...
ast_url <- "https://github.com/Crick-CancerGenomics/ascat/archive/v2.4.3.zip"
ast_path <- tempdir()

ast_filename <- "v2.4.3.zip"
ast_pathname <- file.path(ast_path, ast_filename)

download.file(ast_url, destfile=ast_pathname) 
unzip(ast_pathname, exdir=ast_path)

ast_sourcePath <- file.path(ast_path, "ascat-2.4.3")
ast_sourcePath2 <- paste(ast_sourcePath,"/ASCAT/R/ascat.R", sep = "")
source(ast_sourcePath2)
