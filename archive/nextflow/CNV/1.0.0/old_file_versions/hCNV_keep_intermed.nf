...
process hCNV_get_model_gender {
  ...
  publishDir "${sample_outdir}_tmp", pattern: "*", mode: 'copy'
  ...
  /apt_2.11.3_linux_64_bit_x86_binaries/bin/apt-probeset-genotype -c ${params.snp6chip} -a birdseed --read-models-birdseed ${params.snp6chip_birdseed_mod} --special-snps ${params.snp6chip_specsnps} --out-dir ${sample_outdir}_tmp  --cel-files listfile

  cat ${sample_outdir}_tmp/birdseed.report.txt | grep -v "#" | awk 'NR==2' | cut -f2 > gender_birdseed.txt
  ...
  cat ${sample_outdir}_tmp/birdseed.confidences.txt | grep -v "#" | cut -f1-2 > birdseed.confidences1.txt

  cat ${sample_outdir}_tmp/birdseed.calls.txt | grep -v "#" | cut -f1-2 > birdseed.calls1.txt

  cat ${sample_outdir}_tmp/birdseed.report.txt | grep -v "#" | head -2 > birdseed.report1.txt
...
process hCNV_LRRBAF {
  ...
  /apt_2.11.3_linux_64_bit_x86_binaries/bin/apt-probeset-summarize --cdf-file ${params.snp6chip} --analysis quant-norm.sketch=50000,pm-only,med-polish,expr.genotype=true --target-sketch ${params.hapmap_norm_target} --out-dir ${sample_outdir}_tmp  --cel-files ${cel_list_file}

  ${params.norm_geno_clust} ${params.genoclust} ${sample_outdir}_tmp/quant-norm.pm-only.med-polish.expr.summary.txt -locfile ${params.gw6_pfb_file} -out lrr_baf.txt
...
process hCNV_transfer_files {
  ...
  mv "../../../${sample_outdir}_tmp" "../../../${sample_outdir}"
