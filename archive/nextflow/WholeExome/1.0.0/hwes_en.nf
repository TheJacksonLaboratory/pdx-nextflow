...
def setParamDefaults() {
    ...
    params.targets_gatk         = null // absolute path to targets bed file for gatk
    params.dbsnp                = null // absolute path to dbSNP vcf file
    params.targets_picard       = null // absolute path to targets bed file for picard
    params.targets_exons_picard = null // absolute path to targets exons bed file for picard
    ...
    params.rec_var              = null // absolute path to recurring variants file
    params.bins_hexcoverage     = null // absolute path to coverage bed file
    params.hex_genes            = null // absolute path to genes bed file
...
def helpMessage() {
    ...
    Mandatory:
        ...
        --targets_gatk            absolute path to targets bed file for gatk
        --dbsnp                   absolute path to dbSNP vcf file
        --targets_picard          absolute path to targets bed file for picard
        --targets_exons_picard    absolute path to targets exons bed file for picard
        ...
        --rec_var                 absolute path to recurring variants file
        --bins_hexcoverage        absolute path to coverage bed file
        --hex_genes               absolute path to genes bed file
...
if ( ! params.targets_gatk ) {
    exit 1, "Parameter ERROR: ($params.targets_gatk) absolute path to targets bed file for gatk must be specified."
}
if ( ! params.dbsnp ) {
    exit 1, "Parameter ERROR: ($params.dbsnp) absolute path to dbSNP file must be specified."
}
if ( ! params.targets_picard ) {
    exit 1, "Parameter ERROR: ($params.targets_picard) absolute path to targets bed file for picard must be specified."
}
if ( ! params.targets_exons_picard ) {
    exit 1, "Parameter ERROR: ($params.targets_exons_picard) absolute path to targets exons bed file for picard must be specified."
}
...
if ( ! params.rec_var ) {
    exit 1, "Parameter ERROR: ($params.rec_var) absolute path to recurring variants file must be specified."
}
if ( ! params.bins_hexcoverage ) {
    exit 1, "Parameter ERROR: ($params.bins_hexcoverage) absolute path to coverage bed file must be specified."
}
if ( ! params.hex_genes ) {
    exit 1, "Parameter ERROR: ($params.hex_genes) absolute path to genes bed file must be specified."
}
...
println ".  targets bed file for gatk:         ${params.targets_gatk}"
println ".  dbSNP vcf file:                        ${params.dbsnp}"
println ".  targets bed file for picard:       ${params.targets_picard}"
println ".  targets exons bed file for picard: ${params.targets_exons_picard}"
...
println ".  recurring variants file: ${params.rec_var}"
println ".  coverage bed file:       ${params.bins_hexcoverage}"
println ".  genes bed file:          ${params.hex_genes}"
...
// Step 2a: Xenome
process hwes_en_xenome_classification_a {
  tag "sampleID"
  label 'long_high_mem'
  label 'xenome'
...
process hwes_en_xenome_classification_b {
  tag "sampleID"
  label 'long_high_mem'
  label 'fq_tools'
...
process hwes_en_variant_preproc_1 {
  tag "sampleID"
  label 'short_mem'
  label 'picard_2_8_1'
...
process hwes_en_variant_preproc_2 {
  tag "sampleID"
  label 'long_mem'
  label 'gatk3'
  ...
  java -Djava.io.tmpdir=$TMPDIR -Xmx20g -jar /GenomeAnalysisTK.jar \
  -I ${dedup_bam} \
  -R ${params.ref_fa} \
...
process hwes_en_variant_preproc_3 {
  tag "sampleID"
  label 'short_mem'
  ...
  python ${params.filt_dna_cov} --hex-percentage ${params.hex_percentage} ${sampleID}_CoverageMetrics.txt
...
process hwes_en_var_call_mutect2 {
  tag "sampleID"
  label 'vlong_mem'
  label 'mutect2'

  publishDir ".", pattern: "*final_mutect_snp_indel_filtered.vcf", mode: 'copy'
  ...
  java -Xmx40g -jar /gatk-4.0.5.1/gatk-package-4.0.5.1-local.jar \
  GetSampleName \
  -I ${bam_realigned} \
  -O tumor_SN.txt
  ...
  java -Djava.io.tmpdir=$TMPDIR -Xmx350g -jar /gatk-4.0.5.1/gatk-package-4.0.5.1-local.jar \
  Mutect2 \
  -R ${params.ref_fa} \
  -I ${bam_realigned} \
  -tumor \${tumorName} \
  ...
  java -Djava.io.tmpdir=$TMPDIR -Xmx96g -jar /gatk-4.0.5.1/gatk-package-4.0.5.1-local.jar \
  FilterMutectCalls \
...
process hwes_en_var_filt {
  tag "sampleID"
  label 'short_mem'
  label 'variant_filtration'

  publishDir ".", pattern: "*variants.DPfiltered.vcf", mode: 'copy'
  ...
  python ${params.AD_min_AF} ${mutect2_filtered} ${sampleID}_mutect_snp_indel_filtered.vcf.DPfiltered.tmp.vcf 15
...
process hwes_en_microindel_calling_a{
  tag "sampleID"
  label 'long_mem'
...
process hwes_en_microindel_calling_b {
  tag "sampleID"
  label 'long_mem'
...
process hwes_en_microindel_filtering {
  tag "sampleID"
  label 'short_mem'
  ...
  python ${params.AD_min_AF} ${indels_dp_filt} ${sampleID}_microIndels.DPfiltered1.vcf.DPfiltered_microindels.tmp.vcf 15
...
process hwes_en_variant_annot {
  ...
  java -jar /snpEff_v4_3/snpEff/SnpSift.jar filter --addFilter "lowQD" 'QD < 2.0'  !{adj_snps_fixed} | \
  ...
  java -jar /snpEff_v4_3/snpEff/SnpSift.jar filter --addFilter  "lowReadPosRankSum" 'ReadPosRankSum < -8' > !{sampleID}_snps.vcf
  
  java -jar /snpEff_v4_3/snpEff/SnpSift.jar filter --addFilter "lowQD" 'QD < 2.0' < !{filtered_indels_DP} | \
  java -jar /snpEff_v4_3/snpEff/SnpSift.jar filter --addFilter  "strandbias" 'FS > 200.0' | \
  java -jar /snpEff_v4_3/snpEff/SnpSift.jar filter --addFilter  "lowReadPosRankSum" 'ReadPosRankSum < -20' > !{sampleID}_indels.vcf 


  cat !{sampleID}_snps.vcf !{sampleID}_indels.vcf > !{sampleID}_temp.vcf
...
process hwes_en_tmb_score {
  ...
  header="chr\\tstart\\tend\\tlength\\tHM"
...
process hwes_en_gatk_stats_a {
  tag "sampleID"
  label 'short_high_mem'
  ...
  java -Djava.io.tmpdir=$TMPDIR -Xmx60g -jar /GenomeAnalysisTK.jar \
  -T DepthOfCoverage \
  -R ${params.ref_fa} \
  --outputFormat table \
  ...
  ${params.gatk_form} ${sampleID}_gatk_temp1.txt ${sampleID}_gatk_temp2.txt ${sampleID}_gatk_temp3.txt  ${params.hex_genes}
...
process hwes_en_finalization {
  ...
  if [ "${params.preserve_work}" == "no" ]; then
    ...
    rm -f raw_snps.vcf.idx
    rm -f filtered_indels.vcf.idx 
    rm -f filtered_snps.vcf.idx 
    rm -f *.bai 
    rm -f out.hla.top 
    rm -f out.log.bwamem 
    rm -f out.log.hla 
    rm -f out.hla.all 
    rm -f *_fixAdjSNP.vcf.gz 
    rm -f *.noIds.vcf.gz 
    rm -f *_fixAdjSNP.vcf.gz.tbi
    rm *microIndels_all.vcf
    rm *microIndels.DPfiltered1.vcf
    rm *microIndels.raw.vcf
    rm  -f *.bam
    rm  -rf  ./work
  fi
