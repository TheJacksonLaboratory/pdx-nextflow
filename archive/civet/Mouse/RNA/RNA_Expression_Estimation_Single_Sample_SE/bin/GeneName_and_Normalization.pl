...
#########################gene_names_for_ENSEMBL_ID.pl#############################

        usage: gene_names_for_ENSEMBL_ID.pl [options]
...
while(my $readFileACC = <FILEACC>)
{
  if($readFileACC =~ /^\s*.*?\s+(.*?)\s+(.*?)\s+(.*)$/)
  {
    ...
    {
       $hashGene{$key1} = "$hashGene{$key1}".":"."$UCSC";
    }
...
{ 
  if($flagGene == 0 )
  {
    $flagGene = 1;
    chomp $readFileGene;
    print FILEOUTGENEName "$readFileGene\tGeneName\tUCSCName\n";
    print FILEOUTGENENorm "gene_id\ttranscript_id(s)\tGeneName\tUCSCName\tnormalized_count\n";
    next;
  }
...
while(my $readFileACC = <FILEACC>)
{
  if($readFileACC =~ /^\s*(.*?)\s+.*?\s+(.*?)\s+(.*)$/)
  {
    ...
    {
       $hashIso{$key2} = "$hashIso{$key2}".":"."$UCSC_I";
    }
...
{ 
  if($flagIso == 0 )
  {
    $flagIso = 1;
    chomp $readFileIso;
    print FILEOUTISOName "$readFileIso\tGeneName\tUCSCName\n";
    print FILEOUTISONorm "transcript_id\tgene_id\tGeneName\tUCSCName\tnormalized_count\n";
    next;
  }
