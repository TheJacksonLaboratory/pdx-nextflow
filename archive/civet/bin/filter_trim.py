...
def parse_args():
    ...
    parser.add_argument('-m', '--min_len_pct', default="70", help=
                ...
                'retain read. (0.0 to 1.0 or percentage 2-100) '
                'Note that the value "1" is treated as 100%%, not '
                '1%% (use 0.01) [70]')
    parser.add_argument('-M', '--min_pct_hq_reads', default=0, help=
                'Minimum percentage of reads classified as High '
                'Quality reads (0.0 to 1.0 or percentage 2-100) '
                'Note that the value "1" is treated as 100%%, not '
                '1%% (use 0.01) [0]')
    ...
    parser.add_argument('-o', '--output', help=
                        'Specifies the name of the output file. Otherwise '
                        'the output file name is created by combining the'
...
def output_stats_single(r1, args, start_time):
    ...
    if args.output is not None:
        ...
        ofn = bn_fq1 + '_stat'
    with open(os.path.join(args.odir, ofn), 'w') as sf:
        print >> sf, 'Input file:'
...
def output_stats_paired(r1, r2, args, start_time):
    ...
    if args.output is not None:
        ...
        ofn = '{0}_{1}_stat'.format(bn_fq1, bn_fq2)
